...
const FINNHUB_KEY  = "d73p731r01qjjol3r9q0d73p731r01qjjol3r9qg"; // get free at finnhub.io
    ...
    if (!fetched) {
      try {
        ...
        if (meta?.regularMarketPrice) {
          ...
          out[asset] = { price, change:+change.toFixed(2), trend:change>=0?"bullish":"bearish", live:true, source:"yahoo" };
          fetched = true;
        }
      } catch {}
    }
    ...
    if (!fetched) {
      try {
        ...
        if (meta?.regularMarketPrice) {
          ...
          const prev  = meta.chartPreviousClose || meta.previousClose || price;
          const change = ((price - prev) / prev) * 100;
          out[asset] = { price, change:+change.toFixed(2), trend:change>=0?"bullish":"bearish", live:true, source:"yahoo2" };
          fetched = true;
        }
      } catch {}
    }
    ...
    if (!fetched) {
      try {
        ...
        if (p?.regularMarketPrice?.raw) {
          ...
          out[asset] = { price, change:+change.toFixed(2), trend:change>=0?"bullish":"bearish", live:true, source:"yahoo_v10" };
          fetched = true;
        }
      } catch {}
    }
...
async function fetchReddit() {
  try {
    const r = await fetchJSON("/api/apewisdom/api/v1.0/filter/all-stocks/page/1", {}, 6000);
    const d = await r.json();
    if (!d?.results?.length) throw 0;
    const map = { OIL:["USO","OIL","WTI"], GOLD:["GLD","GOLD","XAU"], NQ:["QQQ","NQ","TQQQ"] };
    const out = {};
    for (const [asset, syms] of Object.entries(map)) {
      const m = d.results.find(x => syms.includes(x.ticker));
      if (m) {
        const prev = parseInt(m.mentions_24h_ago)||1, curr = parseInt(m.mentions)||0;
        out[asset] = { mentions:curr, trend:curr>=prev?"rising":"falling", delta:Math.round(((curr-prev)/prev)*100), live:true };
      } else {
    ...
    return { data:{ OIL:{mentions:0,trend:"neutral",delta:0,live:false}, GOLD:{mentions:0,trend:"neutral",delta:0,live:false}, NQ:{mentions:0,trend:"neutral",delta:0,live:false} }, live:false };
  }
}
  ...
  if (FINNHUB_KEY && FINNHUB_KEY !== "your_finnhub_key_here") {
    try {
      ...
      const endpoints = [
        ...
        `https://finnhub.io/api/v1/company-news?symbol=USO&from=${getDateStr(-2)}&to=${getDateStr(0)}&token=${FINNHUB_KEY}`,
        `https://finnhub.io/api/v1/company-news?symbol=GLD&from=${getDateStr(-2)}&to=${getDateStr(0)}&token=${FINNHUB_KEY}`,
        `https://finnhub.io/api/v1/company-news?symbol=QQQ&from=${getDateStr(-2)}&to=${getDateStr(0)}&token=${FINNHUB_KEY}`,
      ];
...
async function groq(apiKey, messages, max_tokens=1800) {
  ...
  if (!r.ok) throw new Error(`Groq ${r.status}: ${body?.error?.message || "unknown error"}`);
...
async function analyzeMarket({ fg, reddit, news, apiKey }) {
  ...
  const rdText = Object.entries(reddit).map(([k,v]) => `${k}:${v.mentions} mentions ${v.trend}(${v.delta>0?"+":""}${v.delta}%)`).join(", ");
  return groq(apiKey, [
...
function useMarket(apiKey) {
  const [state, setState] = useState({
    status:"idle", market:null, news:[], reddit:{}, prices:{},
    fg:{value:50,label:"Neutral",live:false},
    newsLive:false, redditLive:false, fgLive:false, pricesLive:false,
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      log(`Prices: OIL=${prices.OIL?.live?prices.OIL.change+"% ("+prices.OIL.source+")":"FAILED"} | GOLD=${prices.GOLD?.live?prices.GOLD.change+"%":"FAILED"} | NQ=${prices.NQ?.live?prices.NQ.change+"%":"FAILED"}`);
      ...
      setState(s => ({
        ...s, fg, reddit, prices, news, newsLive,
        redditLive, fgLive:fg.live,
      ...
      // Auto re-analyze if price conflicts with AI bias
      ...
      if (conflicts.length > 0) {
        log(`⚠ Conflict on ${conflicts.map(a=>a.id).join(",")} — re-analyzing...`);
        const priceContext = ASSETS.map(a =>
          `${a.id}: live price ${prices[a.id]?.change>0?"+":""}${prices[a.id]?.change}% TODAY (${prices[a.id]?.trend?.toUpperCase()})`
        ).join(", ");
        const marketV2 = await analyzeMarket({
          fg, reddit,
          news:[{ title:`LIVE PRICE UPDATE: ${priceContext}. Adjust bias accordingly.` }, ...news.slice(0,5)],
          apiKey,
        });
        log("✅ Re-analysis complete.");
        setState(s => ({ ...s, market:marketV2, prices, status:"live", lastMarketUpdate:new Date(), error:null }));
      } else {
  ...
  useEffect(() => {
    if (!apiKey || apiKey.length <= 20) return;
    refresh(); // fetch on app open only
    return () => {
  ...
  return (
    <div style={{ background:"rgba(6,8,16,0.97)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, padding:"10px 14px", display:"flex", alignItems:"center", gap:12 }}>
      <div style={{ display:"flex", alignItems:"center", gap:5, flexShrink:0 }}>
        <Dot color={live?"#22c55e":"#475569"} pulse={live} size={6}/>
        <span style={{ fontFamily:F, fontSize:9, letterSpacing:2, color:"rgba(255,255,255,0.2)" }}>WSB</span>
      </div>
      {ASSETS.map(a => {
        const d = reddit[a.id]; if (!d) return null;
        const c = d.trend==="rising"?"#22c55e":"#ef4444";
        return (
          <div key={a.id} style={{ display:"flex", alignItems:"center", gap:4 }}>
            <span style={{ fontSize:12 }}>{a.emoji}</span>
            <span style={{ fontFamily:F, fontSize:10, color:c }}>{d.trend==="rising"?"▲":"▼"}{Math.abs(d.delta)}%</span>
            <span style={{ fontFamily:F, fontSize:9, color:"rgba(255,255,255,0.18)" }}>{(d.mentions||0).toLocaleString()}</span>
          </div>
        );
      })}
    </div>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily:F, fontSize:10, color:"rgba(255,255,255,0.2)", marginTop:2 }}>{asset.label}</div>
            </div>
          </div>
          {data && <div style={{ background:bm.bg, border:`1px solid ${bm.border}`, borderRadius:8, padding:"5px 12px" }}><span style={{ fontFamily:F, fontSize:11, letterSpacing:2, color:bm.color }}>{bm.icon} {bm.label}</span></div>}
        </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ background:"rgba(5,7,15,0.98)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:16, padding:16, display:"flex", flexDirection:"column", gap:12 }}>
        ...
        {analysisState.error && <div style={{ fontFamily:F, fontSize:11, color:"#f87171", background:"rgba(239,68,68,0.07)", border:"1px solid rgba(239,68,68,0.15)", borderRadius:8, padding:"8px 12px" }}>{analysisState.error}</div>}
      </div>
            ...
            return (
              <div key={i} style={{ background:imp.bg, border:`1px solid ${imp.border}`, borderRadius:14, padding:14, display:"flex", flexDirection:"column", gap:10, animation:"fadeup .25s ease" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:6 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}><ImpactDot level={h.impact_level}/><span style={{ fontFamily:F, fontSize:9, letterSpacing:2, color:imp.color }}>{imp.label} IMPACT</span></div>
...
function BiasTab({ market, prices }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      {ASSETS.map(a => {
        const d = market?.assets?.[a.id], ub = userBias[a.id];
        ...
        const aiC = BIAS_META[d?.bias]?.color||"#eab308";
        const conflict = d?.bias && p?.trend &&
          ((d.bias==="Bullish"&&p.trend==="bearish")||(d.bias==="Bearish"&&p.trend==="bullish"));
        return (
          <div key={a.id} style={{ background:"rgba(5,7,15,0.98)", border:"1px solid rgba(255,255,255,0.06)", borderTop:`3px solid ${a.color}`, borderRadius:16, padding:16, display:"flex", flexDirection:"column", gap:14, boxShadow:`0 4px 24px ${a.glow}` }}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                      ...
                      {conflict && (
                        <div style={{ background:"rgba(249,115,22,0.12)", border:"1px solid rgba(249,115,22,0.3)", borderRadius:7, padding:"3px 10px" }}>
                          <span style={{ fontFamily:F, fontSize:10, color:"#f97316" }}>⚠ AI vs PRICE CONFLICT</span>
                        </div>
                      )}
                    </div>
                    {conflict && (
                      <div style={{ background:"rgba(249,115,22,0.06)", border:"1px solid rgba(249,115,22,0.15)", borderRadius:9, padding:"8px 12px", fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.45)", lineHeight:1.6 }}>
                        Live price is <span style={{ color:priceColor }}>{p.trend.toUpperCase()}</span> but AI bias is <span style={{ color:aiC }}>{d?.bias?.toUpperCase()}</span>. Hit ↻ for updated analysis.
                      </div>
                    )}
                  </div>
                )}
              ...
              {d?.bias && (
                <div style={{ marginLeft:"auto", background:BIAS_META[d.bias]?.bg, border:`1px solid ${BIAS_META[d.bias]?.border}`, borderRadius:8, padding:"4px 10px", flexShrink:0 }}>
                  <span style={{ fontFamily:F, fontSize:10, color:BIAS_META[d.bias]?.color }}>{BIAS_META[d.bias]?.icon} AI: {d.bias}</span>
                </div>
              )}
            </div>
...
export default function App() {
  ...
  const { status, market, news, reddit, fg, prices, newsLive, redditLive, fgLive,
          lastMarketUpdate, lastNewsUpdate, log, error, refresh, refreshNews }
  ...
  if (!submitted) return <><style>{CSS}</style><KeyScreen onSubmit={k => { try { localStorage.setItem("sf_key",k); } catch {} setApiKey(k); setSubmitted(true); }}/></>;
    ...
    <div style={{ minHeight:"100dvh", background:"#030407", position:"relative" }}>
      ...
      <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", minHeight:"100dvh" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"16px 14px 40px", display:"flex", flexDirection:"column", gap:14 }}>
          {tab==="intel" && (
            <>
              ...
              {market ? (
                <>
                  {ASSETS.map(a => <AssetCard key={a.id} asset={a} data={market.assets?.[a.id]}/>)}
                  {[{icon:"⚡",label:"PAIR TRADE",text:market.pair_trade,color:"#a855f7"},{icon:"🎯",label:"RISK EVENT",text:market.risk_event,color:"#ef4444"},{icon:"📡",label:"MACRO",text:market.macro_summary,color:"#3b82f6"}].map(c => (
                  ...
                  {market.correlation_warning && market.correlation_warning!=="null" && (
                    <div style={{ background:"rgba(249,115,22,0.05)", border:"1px solid rgba(249,115,22,0.18)", borderRadius:14, padding:"12px 16px", display:"flex", gap:10 }}>
                      ...
                      <div style={{ fontFamily:F, fontSize:12, color:"rgba(255,255,255,0.42)", lineHeight:1.7 }}><span style={{ color:"#fb923c" }}>CORRELATION TRAP: </span>{market.correlation_warning}</div>
                    </div>
                  )}
                </>
              ) : !isLoading && (
        ...
        <div style={{ padding:"10px 16px 24px", textAlign:"center", fontFamily:F, fontSize:9, color:"rgba(255,255,255,0.07)", letterSpacing:1, lineHeight:2, borderTop:"1px solid rgba(255,255,255,0.04)" }}>
          FINNHUB · FEAR & GREED · APEWISDOM · GROQ LLAMA 3.3 70B<br/>
