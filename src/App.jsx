...
function getNYTime() {
  ...
  // Some browsers return "24:xx" for midnight — normalise to "00:xx"
...
function getKZStatus(assetId) {
  ...
  const zones = ASSET_KILLZONES[assetId] || [];
  ...
  for (const z of zones) {
    let d = z.start - dec;
    if (d <= 0) d += 24;
    if (d < minUntil) { minUntil = d; next = z; }
  }
  return { active:null, next, minutesUntil:Math.round(minUntil * 60) };
}

// Only dim a zone if it clearly ended earlier today (not Asian wrap zone)
function isZonePast(z, dec) {
  return z.end <= 20 && dec > z.end;
}
...
// Single source of truth used by ALL tabs.
// Live price wins on any conflict. source: "ai" | "live" | "none"
...
async function fetchLivePrices() {
  ...
  for (const [asset, sym] of Object.entries(symbols)) {
    let fetched = false;
    // Try v8 chart endpoint on both Yahoo mirrors
...
// ─── Fear & Greed — 3 sources, stock market first ─────────────────────
function fgLabel(v) {
...
async function fetchFearGreed() {
  // 1) feargreedmeter.com — intraday stock market index
  try {
    const r = await fetchJSON(
      "https://api.allorigins.win/raw?url=" + encodeURIComponent("https://feargreedmeter.com/api/v1/fgi/now"),
      {}, 6000
    );
    const d = await r.json();
    const v = parseInt(d?.fgi?.now?.value ?? d?.value ?? d?.score);
    if (!isNaN(v) && v >= 0 && v <= 100)
      return { value:v, label:fgLabel(v), live:true, source:"FearGreedMeter" };
  } catch {}
  ...
  try {
    ...
    if (!isNaN(v) && v >= 0 && v <= 100)
      return { value:v, label:d?.fear_and_greed?.rating ?? fgLabel(v), live:true, source:"CNN" };
  } catch {}

  // 3) alternative.me — crypto F&G, daily update, last resort
  try {
    const r = await fetchJSON("https://api.alternative.me/fng/?limit=1", {}, 6000);
    const d = await r.json();
    const v = parseInt(d?.data?.[0]?.value);
    if (!isNaN(v) && v >= 0 && v <= 100)
      return { value:v, label:d.data[0].value_classification, live:true, source:"AltMe" };
  } catch {}
...
async function fetchNews() {
  // Finnhub first — most relevant financial headlines
  ...
  // Multi-RSS fallback — parallel fetch
  const RSS_FEEDS = [
  ...
  try {
    ...
    if (allRss.length > 0) {
      ...
      const unique = allRss
        ...
        .sort((a,b) => {
          const da = safeParsePubDate(b.pubDate);
          const db = safeParsePubDate(a.pubDate);
          if (!da && !db) return 0;
          if (!da) return 1;
          if (!db) return -1;
          return da - db;
...
async function analyzeMarket({ fg, momentum, news, apiKey, prices }) {
  ...
  // Build hard price constraint — AI must never contradict a live price
  ...
  const hardRule = constraints
    ? `\n\nHARD RULE — LIVE PRICE OVERRIDES EVERYTHING:\n${constraints}\nNever set bias opposite to live price direction.\n`
    : "";

  // Dynamic bias hints injected directly into the JSON template
  const hint = id => {
    const p = prices?.[id];
    return p?.live && p.trend ? (p.trend === "bullish" ? "Bullish" : "Bearish") : "Neutral";
  };

  return groq(apiKey, [
    {
      role:    "system",
      content: "You are a Wall Street ICT trading analyst. Return ONLY valid JSON. No markdown. Respect the HARD RULE — never contradict live price direction.",
    },
    {
      role:    "user",
      content: `Analyze real-time market data for OIL, GOLD, NQ futures trading.${hardRule}
...
KILLZONE RULES:
- OIL/GOLD: London Open 2-5AM EST or NY AM 7-10AM EST
- NQ: NY Open 9:30-11AM EST or NY PM 1-3PM EST

BIAS MUST MATCH LIVE PRICE:
OIL="${hint("OIL")}" GOLD="${hint("GOLD")}" NQ="${hint("NQ")}"

Return ONLY this JSON (fill every field with real analysis, no placeholders left in output):
{"regime":"<RISK-ON|RISK-OFF|STAGFLATION|UNCERTAINTY|NEUTRAL>","regime_reason":"<sentence>","correlation_warning":"<sentence or null>","dxy_bias":"<Bullish|Bearish|Neutral>","dxy_reason":"<sentence>","session_note":"<sentence>","smart_money_note":"<sentence>","assets":{"OIL":{"bias":"${hint("OIL")}","move_type":"<3-5 words>","approach":"<sentence>","sentiment_edge":"<Bullish|Bearish|Neutral>","crowd_vs_smart":"<With crowd|Against crowd>","smt_signal":"<Confirming|Diverging|Neutral>","smt_note":"<sentence>","dxy_impact":"<Headwind|Tailwind|Neutral>","killzone_edge":"<session>","key_level_bull":"<price>","key_level_bear":"<price>","bullish_real_pct":50,"bullish_trap_pct":50,"bearish_real_pct":50,"bearish_trap_pct":50},"GOLD":{"bias":"${hint("GOLD")}","move_type":"<3-5 words>","approach":"<sentence>","sentiment_edge":"<Bullish|Bearish|Neutral>","crowd_vs_smart":"<With crowd|Against crowd>","smt_signal":"<Confirming|Diverging|Neutral>","smt_note":"<sentence>","dxy_impact":"<Headwind|Tailwind|Neutral>","killzone_edge":"<session>","key_level_bull":"<price>","key_level_bear":"<price>","bullish_real_pct":50,"bullish_trap_pct":50,"bearish_real_pct":50,"bearish_trap_pct":50},"NQ":{"bias":"${hint("NQ")}","move_type":"<3-5 words>","approach":"<sentence>","sentiment_edge":"<Bullish|Bearish|Neutral>","crowd_vs_smart":"<With crowd|Against crowd>","smt_signal":"<Confirming|Diverging|Neutral>","smt_note":"<sentence>","dxy_impact":"<Headwind|Tailwind|Neutral>","killzone_edge":"<session>","key_level_bull":"<price>","key_level_bear":"<price>","bullish_real_pct":50,"bullish_trap_pct":50,"bearish_real_pct":50,"bearish_trap_pct":50}},"pair_trade":"<sentence>","risk_event":"<sentence>","macro_summary":"<two sentences>"}`,
    },
  ], 1800);
}
...
async function analyzeNews({ news, apiKey }) {
  ...
  return groq(apiKey, [
    ...
    {
      role:    "user",
      content: `Analyze these headlines for OIL, GOLD, NQ futures impact.

For each headline provide:
- impact_level: "high" / "medium" / "low"
- assets: array from ["OIL","GOLD","NQ"]
- direction: { OIL, GOLD, NQ } each "bullish" / "bearish" / "neutral"
- reason: one sentence
...
Return JSON:
{"analyzed":[{"title":"","impact_level":"high","assets":["OIL"],"direction":{"OIL":"bullish","GOLD":"neutral","NQ":"neutral"},"reason":"","category":"geopolitical"}],"market_summary":"2 sentences","top_risk":"sentence","top_opportunity":"sentence"}`,
    },
  ], 2200);
}
...
function useMarket(apiKey) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      addLog(`OIL=${prices.OIL?.live ? prices.OIL.change+"%" : "FAIL"} GOLD=${prices.GOLD?.live ? prices.GOLD.change+"%" : "FAIL"} NQ=${prices.NQ?.live ? prices.NQ.change+"%" : "FAIL"}`);
      ...
      setState(s => ({
        ...s, fg, momentum, prices, news, newsLive, momentumLive,
        fgLive:     fg.live,
      ...
      // If any bias still conflicts with live price, re-analyse once
      const conflicts = ASSETS.filter(a =>
        resolveEffectiveBias(market?.assets?.[a.id]?.bias, prices?.[a.id]).overridden
      );

      if (conflicts.length > 0) {
        addLog(`⚠ Conflict on ${conflicts.map(a => a.id).join(",")} — re-analysing...`);
        const priceCtx = ASSETS.map(a =>
          `${a.id}: ${prices[a.id]?.change > 0 ? "+" : ""}${prices[a.id]?.change}% (${prices[a.id]?.trend?.toUpperCase() || "?"})`
        ).join(", ");
        const market2 = await analyzeMarket({
          fg, momentum,
          news: [{ title:`PRICE UPDATE: ${priceCtx}. Match bias to price.`, description:"" }, ...news.slice(0,5)],
          apiKey, prices,
        });
        addLog("✅ Re-analysis done.");
        setState(s => ({ ...s, market:market2, status:"live", lastMarketUpdate:new Date(), error:null }));
      } else {
        setState(s => ({ ...s, market, status:"live", lastMarketUpdate:new Date(), error:null }));
      }
    } catch (e) {
...
const Spinner = ({ size = 18 }) => (
  <div style={{
    ...
    border:"2px solid rgba(255,255,255,0.08)", borderTop:"2px solid #3b82f6",
...
function KZBadge({ assetId }) {
  ...
  return a ? (
    ...
    <div style={{ display:"flex", alignItems:"center", gap:5, background:"rgba(100,116,139,0.07)", border:"1px solid rgba(100,116,139,0.15)", borderRadius:8, padding:"4px 10px" }}>
      ...
      <span style={{ fontFamily:F, fontSize:10, color:"#64748b", letterSpacing:1 }}>
...
function KZCard({ asset }) {
  ...
  const a     = kz.active;
  const zones = ASSET_KILLZONES[asset.id] || [];
  const dec   = getNYDecimal();
    ...
    <div style={{ background:"rgba(5,7,15,0.98)", border:`1px solid ${asset.color}20`, borderTop:`3px solid ${asset.color}`, borderRadius:16, padding:16, boxShadow:`0 4px 20px ${asset.glow}` }}>
      ...
      {a ? (
        ...
        <div style={{ background:"rgba(100,116,139,0.06)", border:"1px solid rgba(100,116,139,0.15)", borderRadius:12, padding:"13px 15px", marginBottom:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.28)", marginTop:5 }}>
              Next: <span style={{ color:kz.next?.color || "#94a3b8" }}>{kz.next?.label}</span>
            </div>
          </div>
          <div style={{ textAlign:"right", flexShrink:0, marginLeft:12 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
        {zones.map(z => {
          ...
          const past     = !isActive && isZonePast(z, dec);
          return (
    ...
    <div style={{ background:"rgba(6,8,16,0.97)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:16, padding:16, display:"flex", alignItems:"center", gap:18 }}>
      <svg width="148" height="80" viewBox="0 0 150 80">
        ...
        {["#ef4444","#f97316","#eab308","#22c55e","#16a34a"].map((c,i) => {
          ...
          const ea = (i*36 + 33 - 90) * Math.PI/180;
      ...
      <div>
        <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:5, flexWrap:"wrap" }}>
          ...
          {source && (
            <span style={{ fontFamily:F, fontSize:8, color:"rgba(255,255,255,0.2)", background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:4, padding:"1px 5px", letterSpacing:1 }}>
              {source.toUpperCase()}
            </span>
          )}
        </div>
...
function DXYStrip({ data }) {
  ...
  const color = b === "Bullish" ? "#ef4444" : b === "Bearish" ? "#22c55e" : "#eab308";
  return (
  ...
  return (
    <div style={{ background:c.bg, border:`1px solid ${c.border}30`, borderLeft:`4px solid ${c.border}`, borderRadius:14, padding:"14px 16px", animation:"fadeup .4s ease" }}>
      ...
      {data.smart_money_note && (
        <div style={{ fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.3)", borderTop:"1px solid rgba(255,255,255,0.07)", paddingTop:9, marginTop:9, lineHeight:1.6 }}>
          💼 {data.smart_money_note}
        </div>
      )}
    </div>
        ...
        <div style={{ padding:"0 16px 18px", display:"flex", flexDirection:"column", gap:12, animation:"fadeup .22s ease" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[
              ...
              const c = f.v?.includes("Bull") || f.v?.includes("With")  ? "#22c55e"
                      : f.v?.includes("Bear") || f.v?.includes("Against") ? "#ef4444"
                      : "#eab308";
              return (
...
function NewsTab({ rawNews, newsLive, lastUpdate, apiKey, onRefreshNews }) {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setAnalysis({ status:"done", data, error:null });
    } catch (e) {
      setAnalysis({ status:"error", data:null, error:e.message });
    }
  };
  ...
  const catColor = {
    geopolitical:"#ef4444", central_bank:"#8b5cf6", economic_data:"#3b82f6",
    earnings:"#22c55e", energy:"#f97316", inflation:"#eab308", currency:"#06b6d4", other:"#64748b",
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ background:"rgba(5,7,15,0.98)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:16, padding:16, display:"flex", flexDirection:"column", gap:12 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontFamily:F, fontSize:10, color:"rgba(255,255,255,0.25)", marginTop:4 }}>
              {lastUpdate
                ? `Updated ${lastUpdate.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})} EST`
                : "Not yet loaded"}
            </div>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            <Dot color={newsLive ? "#22c55e" : "#475569"} pulse={newsLive} size={6} />
            <span style={{ fontFamily:F, fontSize:9, color:newsLive ? "#22c55e" : "#64748b", letterSpacing:1 }}>
              {newsLive ? "LIVE" : "DEMO"}
            </span>
          </div>
        </div>
        ...
        {analysis.error && (
          <div style={{ fontFamily:F, fontSize:11, color:"#f87171", background:"rgba(239,68,68,0.07)", border:"1px solid rgba(239,68,68,0.15)", borderRadius:8, padding:"8px 12px" }}>
            {analysis.error}
          </div>
        )}
      </div>
      ...
      <div style={{ background:"rgba(6,8,16,0.97)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:16, padding:"14px 16px", display:"flex", flexDirection:"column", gap:10 }}>
        <div style={{ fontFamily:F, fontSize:10, letterSpacing:2, color:"rgba(255,255,255,0.25)" }}>
          📰 LIVE HEADLINES ({rawNews?.length || 0})
        </div>
        {!rawNews?.length && (
          <div style={{ fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.25)", textAlign:"center", padding:"12px 0" }}>
          ...
          return (
            <div key={i} style={{ paddingLeft:12, borderLeft:"2px solid rgba(255,255,255,0.07)", display:"flex", flexDirection:"column", gap:3 }}>
              ...
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                {pd && (
                  <span style={{ fontFamily:F, fontSize:9, color:"rgba(255,255,255,0.18)" }}>
                    {pd.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",timeZone:"America/New_York"})} EST
                  </span>
                )}
                {h.source && (
                  <span style={{ fontFamily:F, fontSize:8, color:"rgba(255,255,255,0.2)", background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:4, padding:"1px 5px", letterSpacing:1 }}>
      ...
      {analysis.status === "done" && analysis.data && (
        <div style={{ display:"flex", flexDirection:"column", gap:12, animation:"fadeup .3s ease" }}>
            ...
            return (
              <div key={i} style={{ background:imp.bg, border:`1px solid ${imp.border}`, borderRadius:14, padding:14, display:"flex", flexDirection:"column", gap:10, animation:"fadeup .25s ease" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:6 }}>
                  ...
                  <span style={{ background:`${cc}12`, border:`1px solid ${cc}25`, borderRadius:5, padding:"2px 7px", fontFamily:F, fontSize:8, color:cc, letterSpacing:1 }}>
                ...
                <div style={{ fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.42)", lineHeight:1.6, borderTop:"1px solid rgba(255,255,255,0.06)", paddingTop:8 }}>
                  💡 {h.reason}
                </div>
              </div>
            );
          ...
          {filtered.length === 0 && (
            <div style={{ fontFamily:F, fontSize:12, color:"rgba(255,255,255,0.25)", textAlign:"center", padding:"16px 0" }}>
      ...
      {analysis.status === "idle" && rawNews?.length > 0 && (
        <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:14, padding:"22px 16px", textAlign:"center", display:"flex", flexDirection:"column", gap:8, alignItems:"center" }}>
          ...
          <div style={{ fontFamily:F, fontSize:12, color:"rgba(255,255,255,0.3)", lineHeight:1.7 }}>
            Tap <span style={{ color:"#f97316" }}>AI SCAN</span> to analyze headlines.
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      {ASSETS.map(a => {
        ...
        // Safe pct calculation with all guards
        ...
        if (ub && d) {
          ...
          if (typeof rv === "number" && typeof tv === "number") {
            const total = rv + tv;
            if (total > 0) {
              realPct = Math.round((rv / total) * 100);
              trapPct = 100 - realPct;
            } else {
              realPct = 50;
              trapPct = 50;
        ...
        // Alignment hint only for directional bias
        const showAlignment = ub !== null && (effectiveBias === "Bullish" || effectiveBias === "Bearish");
        const aligned       = showAlignment && ub === effectiveBias;
        ...
        return (
          <div key={a.id} style={{ background:"rgba(5,7,15,0.98)", border:"1px solid rgba(255,255,255,0.06)", borderTop:`3px solid ${a.color}`, borderRadius:16, padding:16, display:"flex", flexDirection:"column", gap:14, boxShadow:`0 4px 24px ${a.glow}` }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              ...
              {effectiveBias && (
                <div style={{ marginLeft:"auto", background:ebm.bg, border:`1px solid ${ebm.border}`, borderRadius:8, padding:"4px 10px", flexShrink:0, maxWidth:165 }}>
                  ...
                  {overridden && d?.bias && (
                    <div style={{ fontFamily:F, fontSize:9, color:"#f97316", marginTop:1 }}>AI: {d.bias} ↺</div>
                  )}
                </div>
              )}
            </div>
            ...
            {ub && (
              <div style={{ display:"flex", flexDirection:"column", gap:10, animation:"fadeup .25s ease" }}>
                {!hasPcts ? (
                  ...
                  <>
                    {[
                      { label:"✅ REAL MOVE", pct:realPct, color:"#22c55e" },
                      { label:"⚡ TRAP RISK", pct:trapPct, color:"#f97316" },
                    ].map(b => (
                      <div key={b.label}>
                    ...
                    <div style={{ background: isReal ? "rgba(34,197,94,0.07)" : "rgba(249,115,22,0.07)", border:`1px solid ${isReal ? "rgba(34,197,94,0.2)" : "rgba(249,115,22,0.2)"}`, borderRadius:10, padding:"12px 14px", fontFamily:F, fontSize:12, color: isReal ? "#4ade80" : "#fb923c", lineHeight:1.7, textAlign:"center" }}>
                      {isReal
                        ? `${ub} looks REAL — ${realPct}% real vs ${trapPct}% trap.`
                        : `${ub} looks like a TRAP — ${trapPct}% trap risk vs ${realPct}% real. Wait.`}
                    </div>
                    {showAlignment && (
                      <div style={{ background: aligned ? "rgba(34,197,94,0.05)" : "rgba(239,68,68,0.05)", border:`1px solid ${aligned ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)"}`, borderRadius:10, padding:"10px 13px", fontFamily:F, fontSize:11, color: aligned ? "#4ade80" : "#f87171", lineHeight:1.6 }}>
                        {aligned
                          ...
                          : `⚠ Your bias (${ub}) conflicts with the ${source === "live" ? "live price" : "AI"} read (${effectiveBias}). High-risk.`}
                      </div>
                    )}
                  </>
                )}
            ...
            {!ub && (
              <div style={{ fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.15)", textAlign:"center", letterSpacing:1 }}>
  ...
  return (
    <div style={{ position:"fixed", bottom:0, left:0, right:0, background:"rgba(4,5,8,0.99)", border:"1px solid rgba(59,130,246,0.2)", borderRadius:"18px 18px 0 0", padding:"22px 18px 36px", zIndex:400, maxHeight:"55dvh", overflow:"auto" }}>
      ...
      {(log || []).slice().reverse().map((l, i) => (
        <div key={i} style={{ fontFamily:F, fontSize:11, color: l.includes("❌") ? "#f87171" : l.includes("✅") ? "#4ade80" : l.includes("🤖") ? "#a78bfa" : "rgba(255,255,255,0.35)", lineHeight:2 }}>
          {l}
        </div>
      ))}
      {!log?.length && (
        <div style={{ fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.18)" }}>No entries yet.</div>
      )}
    </div>
  );
}
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(3,4,7,0.97)", zIndex:300, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:22, padding:28 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:6, alignItems:"center", width:"100%", maxWidth:340 }}>
        {(log || []).slice(-4).map((l, i, arr) => (
          <div key={i} style={{ fontFamily:F, fontSize:11, color: i === arr.length-1 ? "#60a5fa" : "rgba(255,255,255,0.15)", letterSpacing:1, textAlign:"center" }}>
            {l}
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  const {
    status, market, news, momentum, fg, prices,
            ...
            <>
              ...
              {market ? (
                <>
                  ...
                  {/* Guard against both JS null and string "null" */}
          ...
          {tab === "kz" && (
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              <div style={{ background:"rgba(59,130,246,0.05)", border:"1px solid rgba(59,130,246,0.15)", borderLeft:"3px solid #3b82f6", borderRadius:14, padding:"12px 16px" }}>
                ...
                <div style={{ fontFamily:F, fontSize:11, color:"rgba(255,255,255,0.38)", lineHeight:1.7 }}>
                  OIL & GOLD → London Open. NQ → NY Open. Each asset has its own optimal trading window.
                </div>
              </div>
